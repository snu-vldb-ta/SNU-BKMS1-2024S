...
simpledb::simpledb(const std::string &pDirName, int pBlockSize, int pBuffSize, std::string pBuffMgrType) {
  ...
  // if pBuffMgrType == "lru", else "vanilla"
  if(pBuffMgrType == "lru"){
    mBM = std::make_unique<lru_buffer_manager>(mFM.get(), mLM.get(), pBuffSize);
  } else {
    mBM = std::make_unique<vanilla_buffer_manager>(mFM.get(), mLM.get(), pBuffSize);
  }
